...
void Game::callbackLoadBallfilePicker(void* userdata, void* contents, int filter, size_t sizeInBytes)
{
    ...
    else if (std::holds_alternative<SerializerSuccessFromDecoding>(result))
    {
        SerializerSuccessFromDecoding success = std::get<SerializerSuccessFromDecoding>(result);

        // Load the song BPMs and offset.
        if (game->appliedMusicId != success.song)
        {
            game->musicId = success.song;
            game->applyNewEditSong();
            game->appliedMusicId = game->musicId;
        }
